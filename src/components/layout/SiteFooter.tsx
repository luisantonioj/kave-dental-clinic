...
const EXTERNAL_LINK_CLASSES =
  "inline-flex min-h-control items-center text-body text-text-inverse-muted underline decoration-transparent underline-offset-4 transition-colors [transition-duration:var(--motion-duration-fast)] hover:text-action hover:decoration-current focus-visible:outline-none focus-visible:ring-[length:var(--focus-ring-width)] focus-visible:ring-focus active:text-action-active";

export function SiteFooter() {
  const currentYear = new Date().getFullYear();

  return (
    <footer className="border-t border-border-strong bg-surface-inverse text-text-inverse">
      <div className="mx-auto grid w-full max-w-wide gap-card-y px-gutter py-section sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="space-y-stack">
          ...
          <address className="space-y-cluster text-body not-italic text-text-inverse-muted">
      ...
      <div className="border-t border-border-strong">
        <p className="mx-auto w-full max-w-wide px-gutter py-stack text-label text-text-inverse-muted">
